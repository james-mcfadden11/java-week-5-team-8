...
values (623, 'Wayne Family', NOW() + INTERVAL '10 DAY', NOW() + INTERVAL '20 DAY'), 
(624, 'Parker Family', NOW() + INTERVAL '11 DAY', NOW() + INTERVAL '20 DAY'),
(625, 'Kent Family', NOW() + INTERVAL '12 DAY', NOW() + INTERVAL '20 DAY');

/*
 STEP SIX: The Wayne Family called and asked if they could change their reservation to today. 
 Update the from_date to today and the to_date to a week from today.
 */
update reservation
set from_date = NOW(), to_date = NOW() + INTERVAL '7 DAY'
where site_id = (select site_id from reservation where name = 'Wayne Family' and from_date = '10/25/2021');
...
where site_id = (select site_id from reservation where name = 'Kent Family' and from_date = '10/27/2021');
