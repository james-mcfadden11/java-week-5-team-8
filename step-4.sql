...
select park.name, count(campground_id) as number_of_campgrounds
from park
join campground using(park_id)
group by park.name
order by park.name;

-- select the park name, campground name, site number, max occupancy, accessible, max rv length, utilities 
--where the campground name is 'Blackwoods'
...
/*
  ...
  -------------------------------------------------
*/
select campground.name as campground_name, count(reservation_id)
from campground
join site using(campground_id)
join reservation using(site_id)
